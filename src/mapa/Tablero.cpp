...
Objeto* Tablero::obtener_objeto_id(string ID) {
    Objeto* objeto_buscado;

    for(int i=0; i<filas; i++)
        for(int j=0; j<columnas; j++)
            if(hay_casilla({i,j})){
                objeto_buscado = casillas[i][j] ->obtener_objeto(ID);
...
Lista<Objeto*> Tablero::obtener_objetos() {
    Lista<Objeto*> objetos;
    for(int i=0; i<filas; i++){
        for(int j=0; j<columnas; j++){
            if(hay_casilla({i,j}))
                objetos+=casillas[i][j]->obtener_objetos();
        }
    }
    return objetos;
}
